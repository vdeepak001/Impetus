        ...
        if ($stateCouncil && $stateCouncil->exists) {
            foreach ($stateCouncil->courseDetails as $course) {
                $this->selectedCourses[$course->id] = [
                    ...
                    'pass_percentage' => $course->pivot->pass_percentage ?? ['', '', ''],
                    'mrp' => $course->pivot->mrp ?? ['', '', ''],
                    'offer_price' => $course->pivot->offer_price ?? ['', '', ''],
                    'points' => $course->pivot->points ?? ['', '', ''],
                    'valid_days' => $course->pivot->valid_days ?? ['', '', ''],
                ];
            }
        }
    }
