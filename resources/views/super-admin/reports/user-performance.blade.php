...
@section('content')
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 mb-8">
        <form action="{{ request()->url() }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex-1 min-w-[200px]">
                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Course
                    / Module</label>
                <select name="course_id"
                    class="block w-full pl-3 pr-10 py-2.5 text-sm border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            ...
            <div class="w-full md:w-44">
                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">From
                    Date</label>
                <input type="date" name="from_date" value="{{ request('from_date') }}" onclick="this.showPicker()"
            ...
            <div class="w-full md:w-44">
                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">To
                    Date</label>
                <input type="date" name="to_date" value="{{ request('to_date') }}" onclick="this.showPicker()"
                    class="block w-full px-3 py-2.5 text-sm border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white date-input-picker">
            </div>
            ...
            <style>
                .date-input-picker::-webkit-calendar-picker-indicator {
                    ...
                    cursor: pointer;
                    filter: invert(0.5);
                    /* Makes it visible on light/dark themes */
                }
            </style>

            <div class="w-full md:w-44">
                <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Exam
                    Type</label>
                <select name="exam_type"
                    ...
                    <option value="passed" {{ request('exam_type') == 'passed' ? 'selected' : '' }}>Completed (Passed)
                    </option>
                </select>
            </div>
            ...
            <div class="flex gap-2 w-full md:w-auto">
                <button type="submit"
                    class="flex-1 md:flex-none bg-[#0082c8] hover:bg-blue-700 text-white font-bold py-2.5 px-8 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg">
                    Search
                </button>
                <a href="{{ request()->url() }}?state_id={{ request('state_id') }}"
                    class="flex-1 md:flex-none text-center bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-white font-bold py-2.5 px-8 rounded-lg transition-all duration-200">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div id="dashboard-report-view"
        class="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden mb-12">
                ...
                <thead>
                    <tr class="bg-[#0082c8]">
                        ...
                        <th
                            ...
                            RN Number</th>
                        <th
                            class="px-4 py-2 text-xs font-bold text-white uppercase tracking-wider border-b border-blue-400/20 text-left w-[24%] whitespace-nowrap">
                            Module Name</th>

                        @if (!request('exam_type') || request('exam_type') === 'pre')
                            <th
                                class="px-4 py-2 text-xs font-bold text-white uppercase tracking-wider border-b border-blue-400/25 text-center w-[8%] whitespace-nowrap">
                                Pre Test</th>
                        @endif
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse($userAttempts as $attempt)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors duration-150">
                            ...
                            <td class="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                {{ $attempt->course_name }}</td>

                            @if (!request('exam_type') || request('exam_type') === 'pre')
                                <td
                                    class="px-4 py-2 text-sm font-bold text-center whitespace-nowrap {{ $attempt->pre_score != '-' ? 'text-blue-600' : 'text-gray-400' }}">
                            ...
                            @if (!request('exam_type') || request('exam_type') === 'mock')
                                <td
                                    class="px-4 py-2 text-sm font-bold text-center whitespace-nowrap {{ $attempt->mock_score != '-' ? 'text-blue-600' : 'text-gray-400' }}">
                            ...
                            @if (!request('exam_type') || in_array(request('exam_type'), ['final', 'passed']))
                                <td
                                    class="px-4 py-2 text-sm font-bold text-center whitespace-nowrap {{ $attempt->final_score_1 != '-' ? 'text-blue-600' : 'text-gray-400' }}">
                                    {{ is_numeric($attempt->final_score_1) ? round($attempt->final_score_1) . ' %' : $attempt->final_score_1 }}
                                </td>
                                <td
                                    class="px-4 py-2 text-sm font-bold text-center whitespace-nowrap {{ $attempt->final_score_2 != '-' ? 'text-blue-600' : 'text-gray-400' }}">
                                    {{ is_numeric($attempt->final_score_2) ? round($attempt->final_score_2) . ' %' : $attempt->final_score_2 }}
                                </td>
                                <td
                                    class="px-4 py-2 text-sm font-medium text-gray-900 dark:text-white text-center whitespace-nowrap">
                                    {{ $attempt->date_of_completion }}</td>
                            @endif
                        </tr>
                    @empty
                        <tr>
                            <td colspan="{{ !request('exam_type') ? 9 : (request('exam_type') === 'pre' || request('exam_type') === 'mock' ? 5 : 7) }}"
                                class="px-8 py-20 text-center">
                                <p class="text-lg font-semibold text-gray-400 dark:text-gray-500">No user performance data
                                    found for the selected filters.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <thead>
                <tr style="background-color: #e5e7eb !important;">
                    ...
                    <th
                        ...
                        Name</th>
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-align: center !important; text-transform: uppercase !important; width: 70px !important;">
                        RN</th>
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-align: center !important; text-transform: uppercase !important; width: 80px !important;">
                        Mobile No</th>
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-transform: uppercase !important; width: 120px !important;">
                        Mail ID</th>
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-transform: uppercase !important;">
                        Module name</th>
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-align: center !important; text-transform: uppercase !important; width: 80px !important;">
                    ...
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-align: center !important; text-transform: uppercase !important; width: 80px !important;">
                        Time of completion</th>
                    <th
                        style="color: #000000 !important; border: 1px solid #d1d5db !important; padding: 8px 12px !important; font-size: 10px !important; font-weight: bold !important; text-align: center !important; text-transform: uppercase !important; width: 60px !important;">
                        Score (%)</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($userAttempts as $attempt)
                    <tr>
                        ...
                        <td class="border border-gray-300 px-3 py-2 text-[10px] text-left">{{ $attempt->course_name }}
                        </td>
                        <td class="border border-gray-300 px-3 py-2 text-[10px] text-center">
                            {{ $attempt->date_of_completion }}</td>
                        <td class="border border-gray-300 px-3 py-2 text-[10px] text-center">
                            {{ $attempt->time_of_completion }}</td>
