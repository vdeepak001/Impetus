            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                @forelse ($stateCouncils as $stateCouncil)
                    <tr wire:key="state-council-{{ $stateCouncil->id }}">
                        ...
                        @if($viewType === 'modules')
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                {{ is_array($stateCouncil->mrp) ? implode(', ', $stateCouncil->mrp) : ($stateCouncil->mrp ?? '—') }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                {{ is_array($stateCouncil->price) ? implode(', ', $stateCouncil->price) : ($stateCouncil->price ?? '—') }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                {{ is_array($stateCouncil->points) ? implode(', ', $stateCouncil->points) : ($stateCouncil->points ?? '—') }}
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                {{ is_array($stateCouncil->valid_days) ? implode(', ', $stateCouncil->valid_days) : ($stateCouncil->valid_days ?? '—') }}
                            </td>
                        @else
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                {{ is_array($stateCouncil->pass_percentage) ? implode(', ', $stateCouncil->pass_percentage) : ($stateCouncil->pass_percentage ?? '—') }}
                            </td>
                        @endif
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route($routePrefix . '.state-councils.edit', $stateCouncil) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">Edit</a>
