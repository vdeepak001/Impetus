...
<div x-data="{ mobileMenuOpen: false, scrolled: true, contactModalOpen: {{ $contactHasErrors ? 'true' : 'false' }} }" @scroll.window="scrolled = (window.pageYOffset > 50)"
                            ...
                            <div class="flex items-center gap-3">
                                @if ($cartCount > 0)
                                    <a href="{{ route('cart.index') }}"
                                        class="relative inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-800 transition hover:border-logo-light-green hover:text-logo-light-green">
                                        ...
                                        <span
                                            class="absolute -right-1 -top-1 inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-logo-blue px-1 text-xs font-bold text-white">
                                            {{ $cartCount }}
                                        </span>
                                    </a>
                                @endif
                                ...
                                <div x-data="{ userMenuOpen: false }"
                                    ...
                                    <div x-show="userMenuOpen" x-cloak
                                        ...
                                        <form method="POST" action="{{ route('logout') }}">
                                            @csrf
                                            <button type="submit"
                                                class="block w-full rounded-xl px-3 py-2 text-left text-sm font-medium text-slate-700 transition hover:bg-slate-50 hover:text-logo-light-green">
                                                Logout
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @else
                            <div class="flex flex-wrap items-center justify-end gap-2">
                                ...
                                <button type="button" @click="$dispatch('open-login-modal')"
                                    class="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-900 transition-colors hover:border-logo-light-green hover:text-logo-light-green">
                                    Log in
                                </button>
                            </div>
                        @endauth
            ...
            <nav class="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-y-contain px-6 py-6"
                ...
                <div class="space-y-1">
                    ...
                    <a href="{{ route('online.examination') }}" @click="mobileMenuOpen = false"
                        ...
                        @if (request()->routeIs('online.examination')) aria-current="page" @endif>Online Test</a>
                    <button type="button" @click="contactModalOpen = true; mobileMenuOpen = false"
                        class="-mx-3 block w-full rounded-lg px-3 py-2 text-left text-base font-medium leading-7 text-slate-900 hover:bg-slate-50">
                        Contact Us
                    </button>
                </div>
                @if (Route::has('login'))
                    <div class="mt-8 border-t border-slate-200 pt-6">
                        @auth
                            @if (auth()->user()?->role_type === 'user')
                                <span
                                    class="-mx-3 block truncate rounded-lg px-3 py-2.5 text-base font-medium leading-7 text-slate-900">
                                    Hi, {{ auth()->user()->name }}
                                </span>
                                @if ($cartCount > 0)
                                    <a href="{{ route('cart.index') }}"
                                        class="-mx-3 flex items-center justify-between rounded-lg px-3 py-2.5 text-base font-medium leading-7 text-slate-900 hover:bg-slate-50">
                                        <span>Cart</span>
                                        <span
                                            class="inline-flex h-6 min-w-[1.5rem] items-center justify-center rounded-full bg-logo-blue px-2 text-xs font-bold text-white">
                                            {{ $cartCount }}
                                        </span>
                                    </a>
                                @endif
                            @else
                                <a href="{{ url('/dashboard') }}"
                                    class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-medium leading-7 text-slate-900 hover:bg-slate-50">Dashboard</a>
                            @endif
                            <a href="{{ route('profile') }}"
                                class="-mx-3 block rounded-lg px-3 py-2.5 text-base font-medium leading-7 text-slate-900 hover:bg-slate-50">
                                My Profile
                            </a>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="-mx-3 block w-full rounded-lg px-3 py-2.5 text-left text-base font-medium leading-7 text-slate-900 hover:bg-slate-50">
                                    Logout
                                </button>
                            </form>
                        @else
    ...
    <div x-show="contactModalOpen" x-cloak class="fixed inset-0 z-[9999] grid place-items-center p-4 sm:p-6"
        role="dialog" aria-modal="true">
        <div class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm transition-opacity"
            @click="contactModalOpen = false"></div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-5">
                <div class="md:col-span-3 p-6 sm:p-8 flex flex-col justify-between">
                    ...
                    <form method="POST" action="{{ route('contact.inquiries.store') }}" class="space-y-4">
                        @csrf
                        <div>
                            <label class="mb-1 block text-xs font-bold text-slate-700 uppercase tracking-wider">Name
                                <span class="text-rose-500 font-bold">*</span></label>
                            <input type="text" name="contact_name"
                                value="{{ old('contact_name', $contactPrefill['name']) }}" required
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="mb-1 block text-xs font-bold text-slate-700 uppercase tracking-wider">Email
                                    ID <span class="text-rose-500 font-bold">*</span></label>
                                <input type="email" name="contact_email"
                                    value="{{ old('contact_email', $contactPrefill['email']) }}" required
                                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/25" />
                                @error('contact_email')
                                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                <label
                                    class="mb-1 block text-xs font-bold text-slate-700 uppercase tracking-wider">Mobile
                                    # <span class="text-rose-500 font-bold">*</span></label>
                                <input type="text" name="contact_phone"
                                    value="{{ old('contact_phone', $contactPrefill['phone']) }}" required
                                    class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/25" />
                                @error('contact_phone')
                                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                                @enderror
                        ...
                        <div>
                            <label class="mb-1 block text-xs font-bold text-slate-700 uppercase tracking-wider">IHSID
                                <span class="text-slate-400 text-xs font-normal">(Optional)</span></label>
                            <input type="text" name="contact_ihsid"
                                value="{{ old('contact_ihsid', $contactPrefill['ihsid']) }}"
                                class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/25" />
                            @error('contact_ihsid')
                                <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label class="mb-1 block text-xs font-bold text-slate-700 uppercase tracking-wider">Query
                        ...
                        <div class="pt-2">
                            <button type="submit"
                                class="w-full rounded-full bg-logo-light-green px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-logo-light-green focus:outline-none focus:ring-2 focus:ring-logo-light-green focus:ring-offset-2">
                                Submit Query
                            </button>
                        </div>
                    </form>
                ...
                <div
                    class="md:col-span-2 bg-gradient-to-br from-logo-blue to-slate-900 text-white p-6 sm:p-8 flex flex-col justify-between relative overflow-hidden">
                    <div
                        class="absolute -right-10 -bottom-10 w-36 h-36 bg-orange-500/15 rounded-full blur-2xl pointer-events-none">
                    </div>
                    <div
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-xs sm:text-sm text-slate-300 leading-relaxed mb-8">Feel free to reach out to us
                            directly through any of these communication channels.</p>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm font-semibold text-slate-200 mt-2 space-y-1.5 leading-relaxed">
                                        <p><a href="tel:+919361737318"
                                                class="hover:text-orange-400 transition-colors">+91 9361737318</a></p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-slate-200 mt-2 leading-relaxed">
                                        <a href="mailto:support@venturacpd.com"
                                            class="hover:text-orange-400 transition-colors">support@venturacpd.com</a>
                                    </p>
                                </div>
                            </div>
                        </div>
